...
def countFingers(image, hand_landmarks, handNo=0):
	...
	if hand_landmarks:
     ...
     finger_x = int((landmarks[8].x)*width)
        finger_y = int((landmarks[8].y)*height)
  
        thumb_x=int((landmarks[4].x)*width)
        thumb_y=int((landmarks[4].y)*height)
  
        cv2.line(image,(finger_x,finger_y),(thumb_x,thumb_y)(255,0,0),2)
  
		

		
		center_x=int((finger_x + thumb_x)/2)
        center_y=int((finger_x + thumb_x)/2)
        
        cv2.circle(image,(center_x , center_y),2,(0,0,255),2)

		# Calculate DISTANCE between FINGER TIP and THUMB TIP
		distance =math.sqrt(((finger_x - thumb_x)**2) + ((finger_y - thumb_y)**2) )
